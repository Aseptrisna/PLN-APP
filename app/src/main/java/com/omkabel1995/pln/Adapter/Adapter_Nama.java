...
public class Adapter_Nama extends RecyclerView.Adapter<Adapter_Nama .MyViewHolder> {
    Context context;
    List<Model_Pln> Menu;

    public Adapter_Nama (Context context, List<Model_Pln> Data_Menu) {
    ...
    @NonNull
    @Override
    public Adapter_Nama.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.list_nama, parent, false);
        Adapter_Nama .MyViewHolder holder = new Adapter_Nama .MyViewHolder(view);
        return holder;
    }

    @Override
    public void onBindViewHolder(@NonNull Adapter_Nama .MyViewHolder holder, int position) {
...
//        });
        holder.namapln.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(context, Menu_DataAsset.class);
                intent.putExtra("ID", Menu.get(position).getId());
                intent.putExtra("NAMA", Menu.get(position).getNama());
//                intent.putExtra("PENGIRIM", Menu.get(position).getPengirim());
//                intent.putExtra("PENERIMA", Menu.get(position).getPenerima());
